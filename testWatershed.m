...
multi_threshold = multithresh(filtered_image,3);
multi_thres=multithresh(filtered_image,2);
BW = imbinarize(filtered_image,otsu_threshold);
MT1=imbinarize(filtered_image,multi_thres(1));
MT2=imbinarize(filtered_image,multi_thres(2));
...
% plot([multi_thres(1) multi_thres(1)],[0 max_histogram],'--b'); % Add a vertical red (dashed) line.
% plot([multi_thres(2) multi_thres(2)],[0 max_histogram],'--b'); % Add a vertical red (dashed) line.
% hold off;
...
gmag=imgradient(filtered_image);
figure(100)
imshow(gmag,[])


se = strel('disk',25);
Io=imopen(filtered_image,se);
Ioc = imclose(Io,se);
Ie=imerode(filtered_image,se);
Iobr=imreconstruct(Ie,filtered_image);
...
Iobr_filtered = imfilter(Iobr,element);

% figure(102)
% subplot(1,2,1)
% imshow(Iobr)
% subplot(1,2,2)
% imshow(Iobr_filtered)

% otsu_threshold = graythresh(Iobr);
% multi_threshold = multithresh(Iobr,2);
% 
% 
% [histogram1,bins] = hist(filtered_image(:),0:0.01:1);
% [histogram2,bins] = hist(Iobr(:),0:0.01:1);
% [histogram3,bins] = hist(Iobrcbr(:),0:0.01:1);
% histogram1(1)=0;
% histogram2(1)=0;
% historgam3(1)=0;
% 
...
MT1_I=imbinarize(Iobrcbr,multi_threshold(1));
MT2_I=imbinarize(Iobrcbr,multi_threshold(2));
MT_dif12_I=MT1_I-MT2_I;
MT1neg_I=imcomplement(MT1_I);


MT2_bg_I=MT2_I-complement_image;
MTdif12_bg_I=MT_dif12_I-complement_image;
MT1neg_bg_I=MT1neg_I-complement_image;
